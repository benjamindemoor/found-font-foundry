...
// Search params wrapper component with the useSearchParams hook
function SearchParamsWrapper({ children }: { children: (pageParam: string | null) => React.ReactNode }) {
  const searchParams = useSearchParams();
  const pageParam = searchParams.get('page');
  return <>{children(pageParam)}</>;
}

// Main content component that uses page parameters from the parent component
function HomeContent({ initialPage }: { initialPage: number }) {
...
export default function Home() {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <SearchParamsWrapper>
        {(pageParam) => (
          <HomeContent initialPage={pageParam ? parseInt(pageParam) : 1} />
        )}
      </SearchParamsWrapper>
    </Suspense>
  );
